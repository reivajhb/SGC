...
while ($mostrarProveedor = mysqli_fetch_array($ejecutar)) {
  ...
  echo "<tr>\n<td>
    <div class='d-flex flex-column align-items-center'>
      <!-- Botón Editar -->
      <a href='modificarProveedoresPrepago.php?id_anticipo={$id_anticipo}'>
      ...
      <button type='button' class='btn btn-danger btn-sm mb-2' data-toggle='modal' data-target='#exampleModalCenter{$id_anticipo}' title='Eliminar'>
    ...
    <!-- Modal Eliminar -->
    <div class='modal fade' id='exampleModalCenter{$id_anticipo}' tabindex='-1' role='dialog' aria-labelledby='modalLabel{$id_anticipo}' aria-hidden='true'>
      <div class='modal-dialog modal-dialog-centered' role='document'>
        <div class='modal-content'>
          ...
          <div class='modal-body'>
            <table class='table'>
              <thead><tr><th>ID</th><th>Proveedor</th><th>Valor</th></tr></thead>
              <tbody>
                <tr>
                  <td>{$id_anticipo}</td>
          ...
          <div class='modal-footer'>
            ...
            <a href='EliminarProveedoresPrepago.php?id_anticipo={$id_anticipo}'>
  ...
  echo '<td>' . htmlspecialchars($mostrarProveedor['identificacion'] ?? '') . '</td>';
  echo '<td>' . htmlspecialchars($mostrarProveedor['fecha'] ?? '') . '</td>';
  echo '<td>' . htmlspecialchars($mostrarProveedor['proveedor'] ?? '') . '</td>';
  echo '<td>' . htmlspecialchars($mostrarProveedor['descripcion'] ?? '') . '</td>';
  echo '<td>' . htmlspecialchars($mostrarProveedor['moneda'] ?? '') . '</td>'; 
  echo '<td>' . htmlspecialchars($mostrarProveedor['localizador'] ?? '') . '</td>';
  echo '<td>$' . $numero . '</td>';
  echo '<td><a href="' . htmlspecialchars($mostrarProveedor['soportePrepago'] ?? '#') . '" target="_blank"><img width="50" height="50" src="/facturacion/img/factura.png" alt="Soporte"></a></td>';
  echo '<td>' . htmlspecialchars($mostrarProveedor['estado'] ?? '') . '</td>';
  echo '<td>' . htmlspecialchars($mostrarProveedor['fecha_lmtpago'] ?? '') . '</td>';
  echo '</tr>';
}
