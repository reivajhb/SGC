...
$id_tiquete_resum = $_GET['id_tiquete_resum'];

// Consulta para recuperar los datos del registro que será eliminado
$consulta_registro = "SELECT * FROM tbl_resum_tiquetes WHERE id_tiquete_resum = '$id_tiquete_resum'";
...
if ($resultado_registro && mysqli_num_rows($resultado_registro) > 0) {
    ...
    $tabla_afectada = "tbl_resum_tiquetes";
    ...
    $sentencia_auditoria = "INSERT INTO auditoria_eliminar (id_registro, tabla_afectada, usuario, datos_eliminados) 
                            VALUES ('$id_tiquete_resum', '$tabla_afectada', '$usuario_eliminacion', '$datos_eliminados')";
    ...
    if ($resultado_auditoria) {
        ...
        $sentencia_eliminar = "DELETE FROM tbl_resum_tiquetes WHERE id_tiquete_resum = '$id_tiquete_resum'"; 
        $resultado_eliminar = mysqli_query($conn, $sentencia_eliminar);
        
        if ($resultado_eliminar) {
            echo '<script>
                      alert("Tiquete eliminado con exito");
    ...
    echo '<script>alert("Error: No se encontraron datos para el registro con ID ' . $id_tiquete_resum . '")</script>';
}
?>
