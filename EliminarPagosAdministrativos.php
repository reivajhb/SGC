...
$id_pago_ad = $_GET['id_pago_ad'];

// Consulta para recuperar los datos del registro que será eliminado
$consulta_registro = "SELECT * FROM tbl_pagos_administrativos WHERE id_pago_ad = '$id_pago_ad'";
...
if ($resultado_registro && mysqli_num_rows($resultado_registro) > 0) {
    ...
    $tabla_afectada = "tbl_pagos_administrativos";
    ...
    $sentencia_auditoria = "INSERT INTO auditoria_eliminar (id_registro, tabla_afectada, usuario, datos_eliminados) 
                            VALUES ('$id_pago_ad', '$tabla_afectada', '$usuario_eliminacion', '$datos_eliminados')";
    ...
    if ($resultado_auditoria) {
        ...
        $sentencia_eliminar = "DELETE FROM tbl_pagos_administrativos WHERE id_pago_ad = '$id_pago_ad'"; 
        $resultado_eliminar = mysqli_query($conn, $sentencia_eliminar);
        
        if ($resultado_eliminar) {
            echo '<script>
                      alert("Pago Administrativo eliminado con exito");
